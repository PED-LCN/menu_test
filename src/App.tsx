...
import { ShoppingBag, Clock, MapPin, Info, ChevronRight, Plus, Minus, Search, X, ArrowLeft } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#F9F7F4] font-sans text-stone-800 pb-28">
      ...
      <div className={`fixed top-0 left-0 right-0 z-50 bg-white shadow-sm transition-transform duration-300 ${isScrolled || isSearchOpen ? 'translate-y-0' : '-translate-y-full'}`}>
        <div className="max-w-3xl mx-auto px-4 h-14 flex items-center gap-3">
          {isSearchOpen ? (
            <div className="flex-1 flex items-center gap-2">
              ...
              />
              {searchTerm && (
                <button onClick={() => setSearchTerm('')} className="p-2 text-stone-400">
                  <X className="w-4 h-4" />
                </button>
              )}
            </div>
            ...
            <div className="flex-1 flex items-center justify-between">
              ...
              <button onClick={toggleSearch} className="p-2 text-stone-600 hover:bg-stone-100 rounded-full">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 mt-6 space-y-10">
        {searchTerm ? (
          <div>
            <h2 className="text-xl font-bold text-stone-800 mb-4">Resultados para "{searchTerm}"</h2>
            {filteredProducts.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredProducts.map(product => (
                  <ProductCard key={product.id} product={product} cart={cart} addToCart={addToCart} removeFromCart={removeFromCart} onSelect={setSelectedProduct} />
                ))}
              </div>
            ) : (
          ...
          <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4">
            ...
            >
              ...
              <div className="p-6 overflow-y-auto">
                ...
                <p className="text-stone-500 mt-2 leading-relaxed">{selectedProduct.description}</p>
                
                {selectedProduct.ingredients && selectedProduct.ingredients.length > 0 && (
                  <div className="mt-6">
                    <h3 className="font-bold text-stone-800 text-sm uppercase tracking-wider">Ingredientes</h3>
                    <div className="flex flex-wrap gap-2 mt-3">
                      {selectedProduct.ingredients.map((ing, idx) => (
                        <span key={idx} className="bg-stone-100 text-stone-600 px-3 py-1 rounded-full text-xs font-medium">
                          {ing}
                        </span>
                      ))}
                    </div>
                ...
                <div className="mt-8 flex items-center justify-between border-t border-stone-100 pt-6">
                  ...
                  <div className="flex items-center gap-4 bg-stone-50 border border-stone-200 rounded-full p-1.5 shadow-sm">
                    ...
                    <span className="font-bold text-lg w-6 text-center">{cart[selectedProduct.id] || 0}</span>
                    <button 
...
function ProductCard({ product, cart, addToCart, removeFromCart, onSelect }: { 
  product: Product, 
